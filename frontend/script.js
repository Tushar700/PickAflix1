...
document.getElementById('mode-search').addEventListener('click', () => {
  ...
  document.getElementById('recommend-mode').classList.add('hidden');
});
...
document.getElementById('mode-recommend').addEventListener('click', () => {
  ...
  document.getElementById('recommend-mode').classList.remove('hidden');
  document.getElementById('search-mode').classList.add('hidden');
  document.getElementById('cbt-input').focus();
});

/* ── CBT SEARCH ─────────────────────────────────── */
document.getElementById('btn-cbt-go').addEventListener('click', getCBTRecommendations);
document.getElementById('cbt-input').addEventListener('keydown', e => {
  if (e.key === 'Enter') getCBTRecommendations();
});

async function getCBTRecommendations() {
  const query  = document.getElementById('cbt-input').value.trim();
  const status = document.getElementById('cbt-status');
  const grid   = document.getElementById('cbt-grid');
  const header = document.getElementById('cbt-results-header');
  ...
  status.textContent = '⏳ Analyzing movie features and computing similarity...';
  status.className = 'cbt-status loading';
  status.classList.remove('hidden');
  document.getElementById('btn-cbt-go').disabled = true;

  try {
    const res  = await fetch(`${CBT_API}/recommend?movie=${encodeURIComponent(query)}&n=10`);
    const data = await res.json();

    if (data.error) {
      status.textContent = `❌ ${data.error}`;
      status.className = 'cbt-status error';
      document.getElementById('btn-cbt-go').disabled = false;
      return;
    }

    // Show results
    status.classList.add('hidden');
    document.getElementById('cbt-query-name').textContent = data.query;
    header.classList.remove('hidden');
    renderCBTMovies(data.recommendations);

  } catch (err) {
    status.textContent = '❌ Could not connect to recommendation engine. Make sure the Python server is running.';
    status.className = 'cbt-status error';
  }
...
function renderCBTMovies(movies) {
  const grid = document.getElementById('cbt-grid');
  grid.innerHTML = '';

  if (!movies.length) {
    grid.innerHTML = '<p class="no-results">No similar movies found. Try a different title.</p>';
    return;
  }
  ...
  movies.forEach(movie => {
    ...
    const rating = movie.vote_average ? `
      <div class="rating-overlay">
        <svg class="star-icon" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
        <span class="rating-num">${movie.vote_average}</span>
      </div>` : '';
    const simPct = Math.round(movie.similarity * 100);

    card.innerHTML = `
      <div class="movie-poster-wrap">
        ${poster}
        ${rating}
        <span class="sim-badge">${simPct}% match</span>
      </div>
      <div class="movie-card-info">
        <div class="movie-card-title">${esc(movie.title)}</div>
        <div class="movie-card-year">${movie.release_year || 'N/A'}</div>
      </div>`;
